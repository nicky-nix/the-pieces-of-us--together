...
| | |
|---|---|
| **Engine & Platform** | Godot 4.x → Android APK (portrait, 9:16) |
| **Camera & View** | Top-down 3/4 perspective · Stardew-style · Camera2D follows active character |
| **Total days** | 43 days · ~3h/day · ~129 total hours |
| **Scope** | 8–12 hand-crafted rooms · procedural shuffle · personal dialogue · epilogue |
...
- **CharacterBody2D with 4-direction sprite sheets** — down, up, left, right — 3 frames each
- **Camera2D on the active character** — smooth following, clamped to room bounds
- **Portrait viewport — 480×854 or 540×960** — set in Project Settings → Display → Window
- **Y-Sort enabled on your main scene** — makes characters appear "behind" objects lower on screen
...
## 2b. How Controls Work — Tap to Move

There is no D-pad. The screen is the controller. This is simpler to build than virtual buttons and feels much more natural on mobile.

### Tap to Move — NavigationAgent2D

When the player taps the screen, the character smoothly walks to that point and navigates around walls automatically. This uses Godot's built-in pathfinding.

- **`InputEventScreenTouch` in `_input(event)`** — fires when the player lifts their finger; get `event.position`
- **`NavigationAgent2D` node on the character** — set `target_position` to the tapped world position
- **`NavigationRegion2D` covering the walkable floor** — bake the navigation mesh once per room
- **In `_physics_process`, move toward `next_path_position()`** — call `move_and_slide()` each frame
...
Because there is no D-pad, you derive which way the character faces from the movement direction automatically:

- `dir.x > 0.5` → play `walk_right` animation
- `dir.x < -0.5` → play `walk_left` animation
- `dir.y > 0.5` → play `walk_down` animation (moving toward camera)
- `dir.y < -0.5` → play `walk_up` animation
- `velocity == Vector2.ZERO` → play `idle_[last_direction]` animation

### Tap Object to Interact — Floating Prompt

Each interactable object (chest, door, switch, Memory Stone) shows a small floating label when the character is close enough. Tapping the object directly triggers it.

- **`Area2D` on each interactable** — detects when the player character is within range
- **`Label` node above object saying "✦ Tap"** — `visible = false` by default; shown when `body_entered` fires
- **`InputEventScreenTouch` on the object** — detect tap; check distance to player before triggering
- **If close enough, emit `interacted` signal** — the object handles the rest
...
### Character Switch — Heart Button (the only UI button in the game)

The only persistent on-screen button is the heart switch button in the corner. Everything else is tap-to-move and tap-to-interact.

- **`TextureButton` anchored to top-right corner of `CanvasLayer`** — always visible, above the game world
- **On pressed → `GameManager.switch_character()`** — toggles `active_player` between "you" and "her"
- **Inactive character stays in place** — their `NavigationAgent` stops; they play idle animation
- **`Camera2D` tweens smoothly to the newly active character** — use `create_tween().tween_property(...)`

> **What this means for your UI**
> Your HUD is extremely minimal: just the heart switch button and a small memory item counter in the corner. No D-pad, no action button, no joystick. The whole screen is the play area. This feels premium and is much easier to build.
...
|---|---|---|---|
| Interface & Scene system | Navigate editor, create 2D scenes, save & run | 2h | Godot docs "Step by Step" + GDQuest intro |
...
| Signals & connections | Detect area overlaps, object taps, room transitions | 1.5h | "Dodge the Creeps" tutorial covers signals well |
| NavigationAgent2D + touch input | InputEventScreenTouch for tap-to-move; NavigationRegion2D for pathfinding around walls | 2h | Godot docs: NavigationAgent2D + InputEventScreenTouch; bake nav mesh per room |
...
| Functions & `_process` / `_physics_process` | Movement each frame, state checks | 1.5h | Write a script that moves a sprite each frame |
...
| Vectors, velocity, `move_and_slide` | Move toward next navigation point; derive facing from velocity | 2h | Copy the 2D movement template from Godot docs; swap input for `agent.get_next_path_position()` |
...
### 3.3 Tap-to-Move, Interaction & Character Switching (6–7 h)

| Sub-skill | What you build / do | Time | How to learn |
|---|---|---|---|
| Tap-to-move (NavigationAgent2D) | `InputEventScreenTouch` → set `agent.target_position`; walk to tapped point | 2h | Godot docs NavigationAgent2D; add NavigationRegion2D to each room and bake |
| Facing from velocity | Derive walk_down/up/left/right from velocity vector each frame | 0.5h | Compare `velocity.x` vs `velocity.y`; track `last_direction` for idle |
| Character switching | Heart button (TextureButton) toggles `active_player`; inactive agent stops | 1.5h | Two CharacterBody2D scenes; only active one gets touch input |
| Tap-to-interact + floating prompt | Area2D detects player nearby; show Label prompt; tap object triggers it | 1.5h | Area2D `body_entered` shows Label; `_input` checks touch on object bounds |
| Room-bound camera | Camera2D clamped to room; smooth tween to new character on switch | 0.5h | Set camera `limit_left/right/top/bottom`; `create_tween()` on switch |

### 3.4 Roguelike Room System (8–10 h)

*This is the biggest new skill — but done simply it is very achievable.*
...
| Sub-skill | What you build / do | Time | How to learn |
|---|---|---|---|
| Key + locked door | Collect key item → door Area2D checks if player holds key | 1.5h | `GameManager.has_item("key")` → `door.open()` |
...
| Minimal HUD (heart button + counter) | Heart switch button anchored top-right; memory counter top-left; nothing else | 0.5h | TextureButton in CanvasLayer; anchor to corner; no D-pad needed |
| Dialogue manager autoload | Dictionary of lines; typewriter reveal; tap to advance | 2.5h | `DialogueManager.show(trigger_id)` → typewriter Timer |
...
*Top-down sprites need 4-directional frames (down, up, left, right) — slightly different from platformer art.*
...
| Sub-skill | What you build / do | Time | How to learn |
|---|---|---|---|
| Find royalty-free music | 1 soft looping track; 1 warmer track for memory moments | 1h | Pixabay Music, Incompetech — search "calm lo-fi loop" |
...
**Theme: Godot basics · portrait setup · tap-to-move · character switch**
*Learning focus: NavigationAgent2D · touch input · Y-Sort · signals*

| Day | Task |
|---|---|
| Day 1–2 | Install Godot 4. Set viewport to 480×854 (portrait). Complete "Dodge the Creeps" for signals and nodes. (3h) |
| Day 3 | Build a test room: TileMap floor, CharacterBody2D with NavigationAgent2D. Enable Y-Sort. Bake nav mesh. Tap to walk. (3h) |
| Day 4 | Add tap-to-interact: Area2D object with floating "✦ Tap" label that appears when player is near. Tapping it prints "interacted". (3h) |
| Day 5 | Add second character scene (Her). Create heart button that switches active_player. Camera smoothly tweens to active character. (3h) |
| Day 6 | Add PointLight2D to Her. Create a dark room where hidden tiles only appear in her light. Test both characters on phone. (3h) |
| Day 7 | Review all code. Test tap-to-move feel on phone. Adjust speed. Make sure nav mesh bakes correctly. Rest. |

**✦ Checkpoint:** Tap-to-move works. Tap-to-interact works. Character switch works. Core tech proven in portrait.

---

### Week 2 (May 22–28) — Room System
**Theme: Hand-crafted rooms · room loader · roguelike shuffle · door transitions**
*Learning focus: TileMap layouts · RoomManager autoload · scene loading*

| Day | Task |
|---|---|
| Day 1–2 | Design 3 room scenes using TileMap (floor + walls). Add exit Area2D triggers at edges. Bake nav mesh in each. (3h each) |
...
| Day 5 | Design 3 more rooms (6 total), all loadable in random order. Test a full 6-room run. (3h) |
...
### Week 3 (May 29–Jun 4) — Story & Puzzles
**Theme: DialogueManager · Memory Stones · all puzzle types · epilogue scene**
*Learning focus: Autoloads · typewriter effect · light puzzle · symbol match*
...
| Day 3 | Write all dialogue lines — room intros, item finds, Memory Stone moments, epilogue text. (3h) |
...
| Day 6 | Add memory item collectibles (2 rooms) — special pickups that appear in gallery. (3h) |
...
### Week 4 (Jun 5–11) — Art & Visuals
**Theme: Top-down character sprites · tiles · backgrounds · UI polish**
*Learning focus: Aseprite 4-direction animation · tileset creation · particle effects*

| Day | Task |
|---|---|
| Day 1–2 | Draw "You" character: 4-direction walk animation (32×32, 3 frames each). (4h) |
| Day 3 | Draw "Her" character: same structure, different palette, hair, PointLight2D glow effect. (3h) |
| Day 4 | Create tileset: floor, wall, wall-top, door open/closed, pressure switch, crate. (3h) |
| Day 5 | Draw memory item icons and collectible sprites. Style dialogue box and HUD button. (3h) |
...
### Week 5 (Jun 12–18) — Audio & Testing
**Theme: Music · SFX · bug fixing · tap-to-move polish**
*Learning focus: Audacity loops · AudioStreamPlayer · collision debugging*
...
| Day 4–5 | Deep playtest on phone: fix tap-to-move accuracy, fix nav mesh gaps at room edges, test room shuffle edge cases. (6h) |
...
### Week 6 (Jun 19–24) — Export & Buffer
**Theme: APK export · keystore · icon · personal note · backup**
...
> You built a love letter in the shape of a game. That's already magic. Happy birthday to her.

---

## 5. Room Design Reference

Design 8 rooms total — every 5 clears a Memory Stone appears. Repeat twice, then the Epilogue. Each room should be solvable in 1–3 minutes.
...
- [ ] Deep playtest on phone — adjust tap-to-move speed and nav mesh in each room
- [ ] Fix all collision bugs
- [ ] Fix any room shuffle edge cases
- [ ] Ensure dialogue never blocks gameplay
- [ ] Polish tap-to-move feel — adjust speed, nav mesh margins, prompt label position
